...
export function NewsCardGrid({ title, items, className, titleClassName }: { title: string, items: any[], className?: string, titleClassName?: string }) {
  return (
    <motion.section 
    ...
    >
      <div className="container mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {items.map(item => <NewsCard key={item.title} item={item} />)}
        </div>
      </div>
    </motion.section>
  )
