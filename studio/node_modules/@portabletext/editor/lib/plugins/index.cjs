  ...
  return behaviors_index.defineBehavior({
    ...
    guard: ({
      ...
      if (prefixOffsets.focus.offset - prefixOffsets.anchor.offset > 1) {
        ...
        }), inlineObjectBeforePrefixFocus = selector_isSelectionExpanded.getPreviousInlineObject({
          ...snapshot,
          context: {
            ...snapshot.context,
      ...
      if (suffixOffsets.focus.offset - suffixOffsets.anchor.offset > 1) {
        const previousInlineObject = selector_isSelectionExpanded.getPreviousInlineObject(snapshot), previousInlineObjectOffset = previousInlineObject ? util_childSelectionPointToBlockOffset.childSelectionPointToBlockOffset({
          context: {
            schema: snapshot.context.schema,
            value: snapshot.context.value
          },
  ...
  const automaticBlockquoteOnSpace = behaviors_index.defineBehavior({
    ...
    guard: ({
      ...
      if (!selectionCollapsed || !focusTextBlock || !focusSpan)
        return !1;
      const previousInlineObject = selector_isSelectionExpanded.getPreviousInlineObject(snapshot), blockOffset = util_sliceBlocks.spanSelectionPointToBlockOffset({
        context: snapshot.context,
        selectionPoint: {
          path: [{
    ...
    guard: ({
      ...
      if (hrCharacter === void 0)
        return !1;
      const hrObject = config.horizontalRuleObject?.(snapshot.context), focusBlock = selector_isSelectionExpanded.getFocusTextBlock(snapshot), selectionCollapsed = selector_isSelectionExpanded.isSelectionCollapsed(snapshot);
      if (!hrObject || !focusBlock || !selectionCollapsed)
        return !1;
      const previousInlineObject = selector_isSelectionExpanded.getPreviousInlineObject(snapshot), textBefore = selector_getTextBefore.getBlockTextBefore(snapshot), hrBlockOffsets = {
        anchor: {
    ...
    guard: ({
      ...
      const text = event.originEvent.dataTransfer.getData("text/plain"), hrRegExp = /^(---)$|(___)$|(\*\*\*)$/, hrCharacters = text.match(hrRegExp)?.[0], hrObject = config.horizontalRuleObject?.(snapshot.context), focusBlock = selector_isSelectionExpanded.getFocusBlock(snapshot);
    ...
    guard: ({
      ...
      if (!blockOffset)
        return !1;
      const previousInlineObject = selector_isSelectionExpanded.getPreviousInlineObject(snapshot), blockText = util_sliceBlocks.getTextBlockText(focusTextBlock.node), markdownHeadingSearch = /^#+/.exec(blockText), level = markdownHeadingSearch ? markdownHeadingSearch[0].length : void 0, caretAtTheEndOfHeading = blockOffset.offset === level;
...
const oneLineBehaviors = [
  ...
  behaviors_index.defineBehavior({
    ...
    guard: ({
      snapshot,
      event
    }) => !(!selector_isSelectionExpanded.getFocusTextBlock(snapshot) || !util_mergeTextBlocks.isTextBlock(snapshot.context, event.block)),
    actions: [({
      event
    }) => [behaviors_index.execute({
      type: "insert.block",
