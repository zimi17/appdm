...
function VisionGui(props) {
  const {
    ...
    perspectiveStack
  } = usePerspective(), defaultApiVersion = prefixApiVersion(`${config.defaultApiVersion}`), editorQueryRef = useRef(null), editorParamsRef = useRef(null), visionRootRef = useRef(null), customApiVersionElementRef = useRef(null), querySubscriptionRef = useRef(void 0), listenSubscriptionRef = useRef(void 0), [localStorage2] = useState(() => getLocalStorage(projectId || "default")), {
    storedDataset,
    storedApiVersion,
    storedQuery,
    storedParams,
  ...
  }), [defaultDataset, defaultApiVersion, localStorage2]), [dataset, setDataset] = useState(() => datasets.includes(storedDataset) ? storedDataset : datasets.includes(defaultDataset) ? defaultDataset : datasets[0]), [apiVersion, setApiVersion] = useState(() => API_VERSIONS.includes(storedApiVersion) ? storedApiVersion : DEFAULT_API_VERSION), [customApiVersion, setCustomApiVersion] = useState(() => API_VERSIONS.includes(storedApiVersion) ? !1 : storedApiVersion), [perspective, setPerspectiveState] = useState(storedPerspective || "raw"), isValidApiVersion = customApiVersion ? validateApiVersion(customApiVersion) : !0, [url, setUrl] = useState(void 0), [query, setQuery] = useState(() => typeof storedQuery == "string" ? storedQuery : ""), [params, setParams] = useState(() => parseParams(storedParams, t)), [queryResult, setQueryResult] = useState(void 0), [listenMutations, setListenMutations] = useState([]), [error, setError] = useState(void 0), [queryTime, setQueryTime] = useState(void 0), [e2eTime, setE2eTime] = useState(void 0), [queryInProgress, setQueryInProgress] = useState(!1), [listenInProgress, setListenInProgress] = useState(!1), [isQueryRecallCollapsed, setIsQueryRecallCollapsed] = useState(!1), {
    paneSizeOptions,
    isNarrowBreakpoint
  } = usePaneSize({
    visionRootRef
  ...
  }), client = useMemo(() => _client.withConfig({
    apiVersion: isValidApiVersion && customApiVersion ? customApiVersion : apiVersion,
  ...
  }, [queryInProgress, query, dataset, params.parsed, t, perspective, perspectiveStack, customApiVersion, isValidApiVersion, apiVersion, localStorage2, cancelListenerSubscription, client, cancelQuerySubscription]), setPerspective = useCallback((newPerspective) => {
  ...
  }, [localStorage2, handleQueryExecution]), handleChangeDataset = useCallback((evt) => {
    const newDataset = evt.target.value;
    localStorage2.set("dataset", newDataset), setDataset(newDataset), handleQueryExecution({
      dataset: newDataset
    });
  }, [localStorage2, handleQueryExecution]), handleChangeApiVersion = useCallback((evt_0) => {
  ...
  }, [localStorage2, handleQueryExecution]), handleCustomApiVersionChange = useCallback((evt_1) => {
  ...
  }, [localStorage2, handleQueryExecution]), handleChangePerspective = useCallback((evt_2) => {
    const newPerspective_0 = evt_2.target.value;
    setPerspective(newPerspective_0 === "default" ? void 0 : newPerspective_0);
  }, [setPerspective]), handleListenerEvent = useCallback((evt_3) => {
  ...
  }, []), handleListenExecution = useCallback(() => {
    if (listenInProgress) {
  ...
  }, [listenInProgress, params, query, localStorage2, cancelQuerySubscription, handleListenerEvent, cancelListenerSubscription, client]), handleParamsChange = useCallback((value) => {
    setParams(value), localStorage2.set("params", value.raw);
  }, [localStorage2]), getStateFromUrl = useCallback((data) => {
  ...
  }, [datasets, dataset, apiVersion, perspective, toast]), setStateFromParsedUrl = useCallback((parsedUrlObj) => {
  ...
  }, [localStorage2, handleQueryExecution]), handlePaste = useCallback((evt_4) => {
  ...
  }, [getStateFromUrl, setStateFromParsedUrl, toast]), handleKeyDown = useCallback((event) => {
  ...
  useEffect(() => (window.document.addEventListener("paste", handlePaste), window.document.addEventListener("keydown", handleKeyDown), () => {
    window.document.removeEventListener("paste", handlePaste), window.document.removeEventListener("keydown", handleKeyDown);
  }), [handleKeyDown, handlePaste]), useEffect(() => () => {
    cancelQuerySubscription(), cancelListenerSubscription();
  }, [cancelQuerySubscription, cancelListenerSubscription]);
  const handleStudioPerspectiveChange = useEffectEvent((stack) => {
    stack.length > 0 && setPerspective("pinnedRelease");
  });
  useEffect(() => {
    handleStudioPerspectiveChange(perspectiveStack);
  }, [perspectiveStack]);
  const generateUrl = useCallback((queryString, queryParams) => {
  ...
  return /* @__PURE__ */ jsxs(Root, { direction: "column", height: "fill", ref: visionRootRef, sizing: "border", overflow: "hidden", "data-testid": "vision-root", children: [
    ...
    /* @__PURE__ */ jsx(SplitpaneContainer, { flex: "auto", children: /* @__PURE__ */ jsxs(SplitPane, { minSize: 800, defaultSize: window.innerWidth - 275, size: isQueryRecallCollapsed ? window.innerWidth : window.innerWidth - 275, maxSize: -225, primary: "first", children: [
        ...
        SplitPane,
        {
          ...
          children: [
            /* @__PURE__ */ jsx(Box, { height: "stretch", flex: 1, children: /* @__PURE__ */ jsxs(SplitPane, { className: "sidebarPanes", split: "horizontal", defaultSize: isNarrowBreakpoint ? paneSizeOptions.defaultSize : paneSizeOptions.minSize, size: paneSizeOptions.size, allowResize: paneSizeOptions.allowResize, minSize: isNarrowBreakpoint ? paneSizeOptions.minSize : 100, maxSize: paneSizeOptions.maxSize, primary: "first", children: [
              /* @__PURE__ */ jsx(InputContainer, { display: "flex", "data-testid": "vision-query-editor", children: /* @__PURE__ */ jsxs(Box, { flex: 1, children: [
                ...
                /* @__PURE__ */ jsx(VisionCodeMirror, { initialValue: query, onChange: setQuery, ref: editorQueryRef })
              ] }) }),
              /* @__PURE__ */ jsxs(InputContainer, { display: "flex", children: [
                /* @__PURE__ */ jsx(ParamsEditor, { value: params.raw, onChange: handleParamsChange, paramsError: params.error, hasValidParams: params.valid, editorRef: editorParamsRef }),
                /* @__PURE__ */ jsx(VisionGuiControls, { hasValidParams: params.valid, queryInProgress, listenInProgress, onQueryExecution: handleQueryExecution, onListenExecution: handleListenExecution })
              ] })
            ] }) }),
            /* @__PURE__ */ jsx(VisionGuiResult, { error, queryInProgress, queryResult, listenInProgress, listenMutations, dataset, queryTime, e2eTime })
          ]
        }
      ) }),
      /* @__PURE__ */ jsxs(Box, { style: {
        ...
        /* @__PURE__ */ jsx(Button, { mode: "ghost", padding: 2, style: {
          position: "absolute",
          left: -32,
          top: "50%",
          ...
          zIndex: 100,
          pointerEvents: "auto"
        }, onClick: () => setIsQueryRecallCollapsed(!isQueryRecallCollapsed), children: /* @__PURE__ */ jsx("div", { style: {
          display: "flex",
          alignItems: "center",
          height: "100%"
        }, children: isQueryRecallCollapsed ? /* @__PURE__ */ jsx(ChevronLeftIcon, {}) : /* @__PURE__ */ jsx(ChevronRightIcon, {}) }) }),
        /* @__PURE__ */ jsx(QueryRecall, { url, getStateFromUrl, setStateFromParsedUrl, currentQuery: query, currentParams: params.parsed || {}, generateUrl })
      ] })
    ] }) })
  ] });
}
