...
function StructureToolProvider({
  ...
  const [layoutCollapsed, setLayoutCollapsed] = React.useState(!1), source = sanity.useSource(), configContext = sanity.useConfigContextFromSource(source), documentStore = sanity.useDocumentStore(), {
    perspectiveStack
  } = sanity.usePerspective(), S = React.useMemo(() => createStructureBuilder({
    defaultDocumentNode,
    source,
    perspectiveStack
  }), [defaultDocumentNode, source, perspectiveStack]), rootPaneNode = React.useMemo(() => resolveStructure ? resolveStructure(S, {
    ...configContext,
    documentStore,
    perspectiveStack
  }) : S.defaults(), [resolveStructure, S, configContext, documentStore, perspectiveStack]), features = React.useMemo(() => ({
